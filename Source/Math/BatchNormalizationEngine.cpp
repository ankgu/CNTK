...
void BatchNormEngine<ElemType>::Forward(const Mat& in, const Mat& scale, const Mat& bias, double expAvgFactor, double blendFactor, Mat& runMean, Mat& runStdDev,
                                        Mat& out, double epsilon, Mat& saveMean, Mat& saveInvStdDev)
{
    ...
    if (!m_spatial)
    {
        ...
        assert(m_inOutT.GetNumElements() == runStdDev.GetNumRows());
    }
    else
    {
        ...
        assert((m_inOutT.GetNumElements() % runStdDev.GetNumRows()) == 0);
    }
    assert(scale.GetNumCols() == 1);
    assert(bias.GetNumCols() == 1);
    assert(runMean.GetNumCols() == 1);
    assert(runStdDev.GetNumCols() == 1);

    EnsureCompatible();
    ForwardCore(in, scale, bias, expAvgFactor, blendFactor, runMean, runStdDev, out, epsilon, saveMean, saveInvStdDev);
...
protected:
    ...
    {
        in.BatchNormalizationForward(scale, bias, expAvgFactor, blendFactor, runMean, runStdDev, out, epsilon, saveMean, saveInvStdDev);
    }
